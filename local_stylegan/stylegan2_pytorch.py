...
# put this once, right after your imports
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if path not in self._cache:
            ...
            if mx > mn:
                data = (data - mn) / (mx - mn)
            # to tensor once, add channel, make D x H x W layout -> [1,D,H,W]
            data = torch.from_numpy(np.transpose(data, (2, 0, 1))).unsqueeze(0).contiguous()
            self._cache[path] = data

        x = self._cache[path]

        # (optional) very cheap CPU flip; otherwise do augment on GPU in AugWrapper
        if self.aug_prob and random() < self.aug_prob:
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity = 16, transparent = False, attn_layers = [], no_const = False, fmap_max = 512): # 512 fmap_max
        ...
        self.num_layers_z = int(log2(4) - 2)  # Layers for Z dimension # 32 for the Paper, please adapt depth parameter here.
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity = 16, fq_layers = [], fq_dict_size = 256, attn_layers = [], transparent = False, fmap_max = 512):# 512
        ...
        image_size_z = 4 # Layers for Z dimension # 32 for the Paper, please adapt depth parameter here.
...
class Trainer():
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, S, G]):
            ...
            with autocast(enabled=self.fp16):
                ...
                image_batch = next(self.loader).to(self.rank, non_blocking=True)
                if apply_gradient_penalty:
                    image_batch.requires_grad_(True)
                # no requires_grad_ when GP is disabled
                real_output, real_q_loss = D_aug(image_batch, **aug_kwargs)
        ...
        if self.is_main:
            ...
            if self.steps % self.evaluate_every == 0 or (self.steps % 100 == 0 and self.steps < 2500):
                ...
                #with open(str(self.results_dir / self.name / "SUMMARY/model_structures.txt"), 'w') as file:
                    #discriminator_summary = summary(D_aug, input_size=(4, 1, 128, image_size, image_size))
    ...
    def evaluate(self, num = 0, trunc = 1.0):
        ...
        generated_images = generated_images[:, :, 3, :, :]
    ...
    def calculate_fid(self, num_batches):
        ...
        base_dir = Path(self.base_dir).resolve()  # Trainer has self.base_dir
        sys.path.insert(0, str(base_dir))
        from MedicalNet.evaluation import (
            generate_model, process_dataset, load_features_from_npy_gz,
            compute_prd_from_embedding, calculate_Medical_fid
        )
